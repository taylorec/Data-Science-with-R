...
cor(Boston) #strongest positive/negative correlations 
# with medv are lstat, rm and ptratio; lstat and rm are correlated

qplot(lstat, medv, data=Boston) # non-linear relation
qplot(log(lstat), medv, data=Boston) #this is a better graph
...
sum_errors <- c(sum(abs(resid(lm.fit1))), sum(abs(resid(lm.fit2))), sum(abs(resid(lm.fit6))),
		sum(abs(resid(lm.fit8))), sum(abs(resid(lm.fit10)))) 

names(sum_errors) <- c('Simple Reg Model', 'Log Model', 'Mult Reg Model', 
			'Lasso Reg Model', 'Complete Model')

sum_errors # Complete model is best
